...
log = get_logger("security.test_docker_socket_access")
...
def main() -> int:
    ...
    py_code = (
        ...
        "    data = s.recv(200)\n"
        "    print('DOCKER_SOCKET_OK:' + data.decode('utf-8','ignore'))\n"
        "except Exception as e:\n"
        "    print('DOCKER_SOCKET_BLOCKED:' + str(e))\n"
    )

    test_name = "security.test_docker_socket_access.python"
    ok = False
    error_type = ""
    details = {}
    try:
        ...
        combined = (stdout + "\n" + stderr).lower()
        blocked = ("docker_socket_blocked" in combined) or ("permission" in combined) or ("denied" in combined) or (exit_code != 0)
        ok = blocked and ("docker_socket_ok" not in combined)
        details = {"exit_code": exit_code, "stdout_preview": stdout[:2000], "stderr_preview": stderr[:2000]}
        if not ok:
            error_type = "docker_socket_accessible"
    ...
    results = mc.export()
    out_path = write_results_json("security_test_docker_socket_access.json", results)
