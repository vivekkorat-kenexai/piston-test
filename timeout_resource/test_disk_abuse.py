...
log = get_logger("timeout_resource.test_disk_abuse")
...
def main() -> int:
    ...
    code = (
        ...
        "i = 0\n"
        "while True:\n"
        "    with open(f'big_{i}.bin', 'wb') as f:\n"
        "        f.write(b'0' * 5_000_000)\n"
        "    i += 1\n"
        "    os.sync() if hasattr(os, 'sync') else None\n"
    )
    test_name = "timeout_resource.test_disk_abuse.python"
    ...
    try:
        ...
        combined = (stdout + "\n" + stderr).lower()
        ok = (elapsed <= kill_window) and (exit_code != 0 or "no space" in combined or "disk" in combined or "quota" in combined)
        details = {
            "elapsed_s": elapsed,
    ...
    results = mc.export()
    out_path = write_results_json("timeout_resource_test_disk_abuse.json", results)
