...
log = get_logger("timeout_resource.test_memory_abuse")
...
def main() -> int:
    ...
    code = (
        "arr = []\n"
        "while True:\n"
        "    # Attempt to exhaust memory quickly\n"
        "    arr.append('X' * 10_000_000)\n"
    )
    test_name = "timeout_resource.test_memory_abuse.python"
    ...
    try:
        ...
        combined = (stdout + "\n" + stderr).lower()
        ok = (elapsed <= kill_window) and (exit_code != 0 or "memory" in combined or "oom" in combined)
        details = {
            "elapsed_s": elapsed,
    ...
    results = mc.export()
    out_path = write_results_json("timeout_resource_test_memory_abuse.json", results)
