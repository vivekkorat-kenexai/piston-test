...
log = get_logger("timeout_resource.test_process_spawn")
...
def main() -> int:
    ...
    js_code = (
        ...
        "let i = 0;\n"
        "while (true) {\n"
        "  try { cp.spawn('sh', ['-c', 'true'], { stdio: 'ignore' }); } catch (e) {}\n"
        "  i++;\n"
        "}\n"
    )
    ...
    for lang, version, code in (
        ...
        test_name = f"timeout_resource.test_process_spawn.{lang}"
        ...
        try:
            ...
            ok = (elapsed <= kill_window) and (exit_code != 0 or "not permitted" in (stderr.lower()))
            details = {
                "elapsed_s": elapsed,
    ...
    out_path = write_results_json("timeout_resource_test_process_spawn.json", results)
