...
log = get_logger("timeout_resource.test_stdout_flood")
...
def main() -> int:
    ...
    test_name = "timeout_resource.test_stdout_flood.python"
    ...
    try:
        ...
        # Pass if output is truncated or killed quickly (non-zero) within window.
        ok = (elapsed <= kill_window) and (exit_code != 0 or len(stdout) > 0)
        details = {
            "elapsed_s": elapsed,
    ...
    results = mc.export()
    out_path = write_results_json("timeout_resource_test_stdout_flood.json", results)
